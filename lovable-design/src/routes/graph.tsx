...
import { projects, members, notes } from "@/lib/mock-data";

export const Route = createFileRoute("/graph")({
  component: Graph,
  head: () => ({ meta: [{ title: "Graph — FlowBoard" }] }),
});
...
function Graph() {
  return (
    <AppShell
      ...
      toolbar={<Toolbar><ToolButton>Tasks</ToolButton><ToolButton>Notes</ToolButton><ToolButton>Projects</ToolButton><span className="ml-auto text-muted-foreground">37 nodes · 84 edges</span></Toolbar>}
    >
      <div className="flex h-full">
        <aside className="w-56 shrink-0 border-r bg-sidebar/40 p-3 text-[12px]">
          ...
          {["Tasks", "Notes", "Projects", "People", "Labels"].map((f, i) => (
            <label key={f} className="flex items-center gap-2 py-1">
              <input type="checkbox" defaultChecked={i < 3} className="h-3 w-3 accent-primary" /> {f}
            </label>
          ))}
          <div className="mt-5 mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Depth</div>
          <input type="range" defaultValue={2} min={1} max={4} className="w-full accent-primary" />
        </aside>
        ...
        <div className="relative flex-1 overflow-hidden bg-[radial-gradient(circle_at_1px_1px,var(--color-border)_1px,transparent_0)] [background-size:24px_24px]">
          ...
          <Node x="50%" y="50%" label="Core Product" main />
          <Node x="30%" y="30%" label="FB-65" />
          <Node x="70%" y="35%" label="Mobile v2" />
          <Node x="35%" y="70%" label="Q3 Roadmap" />
          <Node x="72%" y="68%" label="FB-90" />
          <Node x="20%" y="20%" label="Lisa" small />
          <Node x="82%" y="22%" label="Authentication" small />
          <Node x="86%" y="80%" label="Sara" small />
        </div>
        ...
        <aside className="w-72 shrink-0 border-l bg-background p-4 text-[12px]">
          ...
          <h3 className="text-[15px] font-semibold tracking-tight">Core Product</h3>
          <p className="mt-1 text-muted-foreground">Workspace · 17 items, 5 notes, 5 members</p>
          <div className="mt-4 space-y-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Connected projects</p>
            {projects.slice(0, 3).map((p) => (
              <div key={p.id} className="flex items-center gap-2 rounded px-2 py-1 hover:bg-hover"><span className="h-1.5 w-1.5 rounded-full" style={{ background: p.accent }} />{p.name}</div>
            ))}
          </div>
          <div className="mt-4">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Connected notes</p>
            {notes.slice(0, 2).map((n) => (
              <div key={n.id} className="rounded px-2 py-1 hover:bg-hover">{n.title}</div>
            ))}
          </div>
          <div className="mt-4">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">People</p>
            {members.slice(0, 3).map((m) => (
              <div key={m.id} className="rounded px-2 py-1 hover:bg-hover">{m.name}</div>
            ))}
          </div>
        </aside>
      </div>
