...
export function ToolButton({ children, active }: { children: ReactNode; active?: boolean }) {
  return (
    <button className={`flex h-7 items-center gap-1.5 rounded px-2 text-[12px] ${active ? "bg-hover text-foreground" : "text-muted-foreground hover:bg-hover hover:text-foreground"}`}>
      {children}
    </button>
  );
}
