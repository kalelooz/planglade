...
const DEFAULT_LOCAL_SIGNING_SECRET = "flowboard-local-storage-dev-secret"
...
function getLocalStorageRootDir() {
  return path.resolve(process.cwd(), process.env.FLOWBOARD_LOCAL_STORAGE_DIR ?? DEFAULT_LOCAL_STORAGE_DIR)
}
...
function getStorageSigningSecret() {
  ...
  return DEFAULT_LOCAL_SIGNING_SECRET
}
...
export async function storageObjectExists(storageKey: string) {
  ...
  try {
    await stat(resolveLocalStoragePath(storageKey))
    return true
  } catch {
    return false
  }
...
export async function readStorageObjectMetadata(storageKey: string): Promise<StoredObjectMetadata | null> {
  ...
  try {
    ...
    const [fileStat, rawMeta] = await Promise.all([
      stat(filePath),
      readFile(getLocalMetaPath(filePath), "utf8").catch(() => null),
    ])
