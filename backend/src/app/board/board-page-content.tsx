...
type DragTarget = { status: Status; index: number };
...
export function reorderLocalWorkItems(
  workItems: WorkItem[],
  id: string,
  targetStatus: Status,
  ...
  const targetProjectId = scopeProjectId ?? moved.project;
  const inScope = (workItem: WorkItem) =>
    workItem.status === targetStatus && workItem.project === targetProjectId;
  ...
  const positions = new Map(
    reordered
      .filter(inScope)
      .map((workItem, index) => [workItem.id, (index + 1) * 1024]),
  );
  return reordered.map((workItem) => {
    const position = positions.get(workItem.id);
    return position === undefined ? workItem : { ...workItem, position };
  });
}
...
export function BoardPageContent() {
  ...
  const setNextDragTarget = (next: DragTarget | null) => {
    dragTargetRef.current = next;
    setDragTarget(next);
  };

  const getDragTarget = (event: DragOverEvent): DragTarget | null => {
    const activeId = String(event.active.id);
    const moved = scopedWorkItems.find((item) => item.id === activeId);
    const status = resolveTargetStatus(event.over, scopedWorkItems);
    if (!moved || !status || !event.over) return null;

    const targetItems = scopedWorkItems.filter((item) => item.status === status && item.project === moved.project && item.id !== activeId);
    if (columnIds.has(String(event.over.id))) return { status, index: targetItems.length };

    const overIndex = targetItems.findIndex((item) => item.id === String(event.over?.id));
  ...
  const patchTaskStatus = async (id: string, status: Status, beforeId: string | null, snapshot?: WorkItem[]) => {
  ...
  const onDragStart = (e: DragStartEvent) => {
    ...
    const item = scopedWorkItems.find((workItem) => workItem.id === id);
    setActiveDragId(id);
    setDragHeight(e.active.rect.current.initial?.height ?? 0);
    setNextDragTarget(item ? { status: item.status, index: scopedWorkItems.filter((workItem) => workItem.status === item.status).indexOf(item) } : null);
  };

  const onDragOver = (e: DragOverEvent) => setNextDragTarget(getDragTarget(e));

  const onDragEnd = (e: DragEndEvent) => {
    if (blockReadOnlyMutation(isDemoMode)) { setActiveDragId(null); setNextDragTarget(null); return; }
    ...
    const targetItems = scopedWorkItems.filter((item) => item.status === target.status && item.project === moved.project && item.id !== activeId);
    ...
    void patchTaskStatus(activeId, target.status, beforeId, workItems);
                ...
                {cols.map((col) => {
                  ...
                  return (
                    <Column
                      ...
                      holeAt={dragTarget?.status === col ? dragTarget.index : null}
                      ...
                      onMove={(id, s) => {
                        ...
                        void patchTaskStatus(id, s, null, snapshot);
                      }}
      ...
      >
        <SortableContext items={visibleItems.map((i) => i.id)} strategy={verticalListSortingStrategy}>
          {visibleItems.map((w, index) => (
            <div key={w.id} data-slotwrap>
              {holeAt === index && <DropHole height={dragHeight} />}
              <Card
                item={w}
          ...
          {holeAt === visibleItems.length && <DropHole height={dragHeight} />}
        </SortableContext>
...
function DropHole({ height }: { height: number }) {
  return (
    <div
      aria-hidden="true"
      data-task-drop-hole="true"
      className="rounded-md border border-dashed border-primary/40 bg-primary/5"
