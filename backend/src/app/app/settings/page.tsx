...
type WorkspaceSnapshot = {
  ...
  data?: {
    ...
    savedViews?: Array<{
      ...
      project?: string;
      name: string;
      layout: "list" | "board" | "kanban" | "calendar" | "map" | "timeline" | "overview" | "table";
      groupBy?: string;
      orderBy?: string;
...
function AppearancePreview({
  ...
  return (
    <div data-priority-preview="live" className="max-w-xl overflow-hidden rounded-md border border-border/80 bg-background">
...
function PreviewStaticMeta({ status }: { status: string }) {
  return (
    <span className="hidden max-w-28 truncate rounded border border-border/80 bg-muted/50 px-1.5 py-0.5 text-[10px] text-muted-foreground sm:inline-flex">
      {status}
    </span>
  );
}
    ...
    try {
      ...
      const importResponse = await apiFetch("/api/workspace/import-local", {
        ...
        body: JSON.stringify({
          ...
          notes: notesPayload,
          projectDocs: parsed.data?.projectDocs ?? [],
          savedViews: parsed.data?.savedViews ?? [],
        }),
      });
    ...
    try {
      const response = await apiFetch("/api/workspace/import-local", {
        ...
        body: JSON.stringify({
          ...
          savedViews: guidedImportSnapshot.data?.savedViews ?? [],
        }),
      });
    ...
    <AppShell title={<span className="font-medium">Settings</span>}>
      <div className="flex h-full min-w-0 flex-col overflow-y-auto md:flex-row">
        <aside className="shrink-0 border-b bg-sidebar/40 p-3 md:w-56 md:border-r md:border-b-0">
          ...
          <div className="flex gap-1 overflow-x-auto pr-8 [mask-image:linear-gradient(90deg,#000_calc(100%-2rem),transparent)] md:block md:space-y-1 md:overflow-visible md:pr-0 md:[mask-image:none]">
            {sections.map((x) => (
                      ...
                      {PRIORITY_DISPLAY_OPTIONS.map((opt) => {
                        return (
                          <RadioGroupPrimitive.Item
                            ...
                            className="flex h-14 min-w-0 items-center justify-between gap-2 rounded-md border border-border/80 bg-background px-3 py-2 text-left text-xs text-muted-foreground transition-colors hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/20 focus-visible:ring-offset-1 data-[state=checked]:border-ring data-[state=checked]:bg-muted/50 data-[state=checked]:text-foreground data-[state=checked]:ring-1 data-[state=checked]:ring-ring/10"
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-4 rounded-md border bg-card p-3">
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                        {[
                          ...
                          ["Saved views", guidedImportPreview.counts.savedViews],
                  ...
                  {guidedImportSummary && (
                    <div className="rounded-md border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 text-[13px]">
                      Import merged. Projects {guidedImportSummary.imported.projects}, tasks {guidedImportSummary.imported.workItems}, notes {guidedImportSummary.imported.notes}, Project Docs {guidedImportSummary.imported.projectDocs}, saved views {guidedImportSummary.imported.savedViews}. Refresh or open a workspace view to see imported items.
                    </div>
                  )}
                </div>
              </Field>
      ...
      <Dialog open={resetConfirmOpen} onOpenChange={setResetConfirmOpen}>
        <DialogContent className="max-w-sm p-5">
          <DialogHeader className="text-left">
            <DialogTitle className="text-[15px] font-semibold">Reset seed data?</DialogTitle>
