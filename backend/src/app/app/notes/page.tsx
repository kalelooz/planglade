...
function NotesInner() {
  ...
  const quickCreate = async () => {
    if (!draft.trim()) {
      setDraftError("Add note text before saving.");
      return;
    }
    setDraftError(null);
    ...
    <AppShell title={<span className="font-medium">Notes</span>}>
      <div className="flex h-full flex-col bg-background md:flex-row">
        <aside className="m-0 flex max-h-[40vh] w-full shrink-0 flex-col rounded-none border-b bg-card md:ml-4 md:mb-4 md:mt-4 md:max-h-none md:w-72 md:rounded-md md:border md:border-b-0">
          <div className="flex items-center gap-2 border-b border-border/40 px-3 py-2">
            ...
            <input
              ...
              className="h-7 flex-1 bg-transparent text-base outline-none placeholder:text-muted-foreground sm:text-[13px]"
          ...
          <div className="border-b border-border/40 p-2">
            <label htmlFor="note-quick-capture" className="mb-1 block text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Capture</label>
            <div className="flex items-center gap-1">
              <input
                ...
                aria-describedby={draftError ? "note-quick-capture-error" : undefined}
            ...
            {draftError && <p id="note-quick-capture-error" className="mt-1 text-[11px] text-red-600">{draftError}</p>}
          </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          {error && <div role="alert" className="mx-5 mt-3 rounded border border-destructive/30 bg-destructive/10 px-3 py-2 text-[12px] text-destructive">Unable to load notes. Refresh the page to try again. Your workspace data is unchanged.</div>}
          {sel ? (
            <>
              <div className="flex min-h-12 flex-wrap items-center gap-2 border-b border-border/40 bg-card/40 px-3 py-2 md:px-5 text-[12px]">
                ...
                <label className="group flex items-center gap-1 rounded-md border border-border bg-card px-2 py-1 hover:border-ring focus-within:border-ring">
                  ...
                  <input
                    ...
                    className="w-24 bg-transparent text-base font-medium text-foreground outline-none sm:text-[12px]"
                    aria-label="Tag"
                  />
