  ...
  return (
    <button
      ...
      style={{
        ...
        width: node.width,
        height: node.height,
        borderColor: selected ? "var(--color-ring)" : related ? "var(--color-muted-foreground)" : undefined,
        boxShadow: selected ? "0 0 0 2px color-mix(in oklch, var(--color-ring) 14%, transparent)" : undefined,
        opacity: dimmed ? 0.28 : 1,
      }}
      aria-label={`Select ${node.type}: ${node.label}${node.meta ? `. ${node.meta}` : ""}`}
      data-graph-node={node.type}
      className={`absolute flex items-center gap-2 rounded-md border bg-background px-3 text-left shadow-sm transition-[border-color,box-shadow,opacity,transform] hover:-translate-y-0.5 hover:bg-muted/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${typeStyle.border}`}
    >
      <span
        className="flex h-7 w-7 shrink-0 items-center justify-center rounded border border-border bg-muted/50 text-muted-foreground"
      >
        {node.type === "person" ? (
    ...
    >
      <div className="flex h-full min-h-0 flex-col overflow-x-hidden">
        <header className="shrink-0 border-b bg-background px-4 py-4">
          <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Workspace graph</div>
          <div className="mt-1 flex flex-wrap items-end justify-between gap-3">
            <div>
              <h1 className="text-[18px] font-semibold tracking-tight text-foreground">Connections</h1>
        ...
        <div className="flex min-h-0 flex-col border-r">
          <div className="flex min-h-12 shrink-0 flex-wrap items-center gap-2 border-b bg-background px-3 py-2">
            <div className="relative w-64 max-w-full">
              ...
              <input
                ...
                className="h-8 w-full rounded border bg-card pl-8 pr-2 text-base outline-none focus:border-ring sm:text-[12px]"
              />
            </div>
          ...
          <div
            ...
            className="relative min-h-0 flex-1 cursor-grab overflow-hidden bg-muted/50 active:cursor-grabbing"
            onWheel={handleWheel}
          ...
          >
            ...
            {loading && (
              <div role="status" className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center bg-muted/85 text-sm text-muted-foreground">
                Loading connections...
              </div>
            )}
            {!loading && error && (
              <div role="alert" className="pointer-events-none absolute inset-0 z-20 flex flex-col items-center justify-center bg-muted/90 px-6 text-center">
                <span className="text-sm font-semibold text-foreground">Unable to load connections</span>
                <span className="mt-1 text-xs text-muted-foreground">Refresh the page to try again. Your workspace data is unchanged.</span>
              </div>
            )}
            {!loading && !error && nodes.length === 0 && (
              <div role="status" className="pointer-events-none absolute inset-0 z-20 flex flex-col items-center justify-center bg-muted/90 px-6 text-center">
                <span className="text-sm font-semibold text-foreground">No connections found</span>
                    ...
                    {relationshipRows.length === 0 ? (
                      <p className="rounded border border-dashed px-3 py-5 text-center text-[12px] text-muted-foreground">No relationships found for the current filters.</p>
                        ...
                        relationshipRows.map((row) => (
                          <article key={row.id} className="rounded border border-border px-2 py-1.5 text-[12px]">
                            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{row.label}</div>
                            <div className="mt-0.5 min-w-0 text-foreground/80">
                              <span>{row.from}</span>
                              {row.fromProject && row.fromProject !== row.from && <span className="text-muted-foreground"> / <span>{row.fromProject}</span></span>}
                              <span aria-hidden="true"> {" -> "} </span>
                              <span>{row.to}</span>
                              {row.toProject && row.toProject !== row.to && <span className="text-muted-foreground"> / <span>{row.toProject}</span></span>}
                            </div>
                          </article>
                        ))
                    )}
