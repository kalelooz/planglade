...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [workspace, projects, workItems, notes, labels, projectDocs, savedViews] = await Promise.all([
    ...
    const serializedWorkItems = workItems.map((item) => ({
      ...
      status: item.status,
      isInbox: item.isInbox ?? item.status === "BACKLOG",
    ...
    const serializedSavedViews = savedViews.map((view) => ({
      id: view.id,
      project: view.projectId ?? undefined,
      name: view.name,
      layout: view.layout,
    ...
    const response = NextResponse.json({
      ...
      savedViews: serializedSavedViews,
      data: {
