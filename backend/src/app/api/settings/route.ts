...
import { forbidden, hasMinimumWorkspaceRole, parseJsonBody, parseQuery, requireWorkspaceRole, resolveRequestActorUserId, serverError } from "@/lib/api-utils"
import { resolvePriorityDisplayStyle, updateUserSettingsSchema, workspaceUserQuerySchema } from "@/lib/contracts"
  ...
  try {
    ...
    const [settings, workspace] = await Promise.all([
      ...
      db.workspace.findUnique({
        ...
        select: { taskPriorityDisplayStyle: true },
      }),
    ])
    return NextResponse.json({
      settings,
      workspace: workspace
        ? { ...workspace, taskPriorityDisplayStyle: resolvePriorityDisplayStyle(workspace.taskPriorityDisplayStyle) }
    ...
    const [settings, workspace] = await Promise.all([
      db.userSettings.upsert({
        ...
        update: {
          ...
          ...(parsed.data.priorityDisplay !== undefined ? { priorityDisplay: parsed.data.priorityDisplay } : {}),
          ...(parsed.data.weekStartsOn !== undefined ? { weekStartsOn: parsed.data.weekStartsOn } : {}),
          ...(parsed.data.hideHomeCompleted !== undefined ? { hideHomeCompleted: parsed.data.hideHomeCompleted } : {}),
        ...
        create: {
          ...
          priorityDisplay: parsed.data.priorityDisplay,
          weekStartsOn: parsed.data.weekStartsOn,
          hideHomeCompleted: parsed.data.hideHomeCompleted,
      ...
      parsed.data.taskPriorityDisplayStyle !== undefined
        ? db.workspace.update({
            ...
            data: { taskPriorityDisplayStyle: parsed.data.taskPriorityDisplayStyle },
            select: { taskPriorityDisplayStyle: true },
          })
        : db.workspace.findUnique({
            where: { id: parsed.data.workspaceId },
