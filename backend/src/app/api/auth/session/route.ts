...
export async function GET(request: Request) {
  try {
    ...
    if (authMode === "dev-session-scaffold") {
      ...
      const defaultWorkspace = await db.workspace.upsert({
      ...
      await db.workspaceMember.upsert({
        ...
        update: { role: "OWNER" },
        create: { workspaceId: defaultWorkspace.id, userId: user.id, role: "OWNER" },
      })
    ...
    const selectedMembership = requestedWorkspaceId
      ? workspaceMemberships.find((membership) => membership.workspaceId === requestedWorkspaceId)
      : workspaceMemberships.find((membership) => membership.workspaceId === defaultWorkspaceId) ?? workspaceMemberships[0]
    if (!selectedMembership) {
      return NextResponse.json({ error: "Workspace access unavailable" }, { status: 403 })
    }
    const workspace = selectedMembership.workspace
    ...
    return NextResponse.json({
      ...
      workspaces: workspaceMemberships.map((membership) => ({
        ...
        taskPriorityDisplayStyle: resolvePriorityDisplayStyle(membership.workspace.taskPriorityDisplayStyle),
