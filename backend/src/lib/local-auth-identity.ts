...
const userSelect = { id: true, email: true, name: true, image: true, authVersion: true } as const satisfies Prisma.UserSelect

export type VerifiedApplicationUser = { id: string; email: string; name: string | null; image: string | null; authVersion: number }
type VerifiedIdentity = { email: unknown; name?: string | null; image?: string | null }
...
export async function resolveVerifiedApplicationUser(identity: VerifiedIdentity): Promise<VerifiedApplicationUser | null> {
  ...
  try {
    return await db.user.create({ data: { email, normalizedEmail, name: identity.name, image: identity.image }, select: userSelect })
  } catch (error) {
    if (isUniqueConstraintError(error)) return null
    throw error
  }
...
export async function resolveLegacyNextAuthUser(email: unknown): Promise<VerifiedApplicationUser | null> {
  ...
  return transitionalUsers.length === 1 && transitionalUsers[0].authVersion === 0 ? transitionalUsers[0] : null
}
