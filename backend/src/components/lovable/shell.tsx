...
import { type ApiProject, toUiNotePreview, toUiProject, toUiWorkItem } from "@/lib/server-ui-mappers";
  ...
  return (
    <div className="flex h-dvh w-full overflow-hidden bg-background text-foreground">
      ...
      >
        ...
        <nav aria-label="Primary navigation" className={`flex-1 overflow-y-auto overflow-x-hidden py-3 ${sidebarOpen ? "px-2" : "px-1"}`}>
          {sidebarOpen ? (
            <>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="relative z-40 flex h-auto min-h-12 shrink-0 items-center gap-2 border-b bg-background/95 px-2 py-2 shadow-[0_1px_0_color-mix(in_oklch,var(--color-primary)_8%,transparent)] sm:gap-3 sm:px-4 sm:py-0">
              ...
              {quickOpen && (
                <div className="absolute right-0 top-9 z-[80] w-80 rounded-md border bg-popover p-2 shadow-lg">
                    <input
                      ...
                      aria-label="Quick capture task or idea"
                      className="h-8 w-full rounded border bg-card px-2 text-base outline-none focus:border-ring sm:text-[13px]"
                    />
          ...
          <button onClick={() => setCmdOpen(true)} aria-label="Open command palette" className="lov-icon-btn h-[44px] w-[44px] sm:hidden">
          ...
          <div ref={notificationsRef} className="relative">
            ...
            {notificationsOpen && (
              <div className="absolute right-0 top-9 z-[85] w-[min(22rem,calc(100vw-1rem))] rounded-md border bg-popover shadow-lg">
      ...
      {mobileNavOpen && (
        <div className="fixed inset-0 z-[90] md:hidden" role="dialog" aria-modal="true" aria-label="Navigation">
          ...
          <div className="absolute inset-y-0 left-0 flex w-72 max-w-[86vw] flex-col border-r bg-sidebar shadow-xl" data-mobile-navigation-drawer="true">
            ...
            <nav aria-label="Mobile navigation" className="flex flex-1 flex-col overflow-y-auto px-2 py-3 [&_a]:min-h-[44px] [&_button]:min-h-[44px] [&_button]:min-w-[44px]">
      ...
      <CommandPalette
        ...
        projects={projects}
        workItems={demoData?.apiTasks.map((item) => toUiWorkItem(item, "demo-user"))}
        notes={demoData?.apiNotes.map(toUiNotePreview)}
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-px">
      {items.map((n) => {
        ...
        if (collapsed) {
          return (
            <Link key={n.to} href={n.to} title={n.label} aria-label={n.label} onClick={onNavigate}
