...
export function FlowRow({
  ...
  const Comp = as;
  const canUseRowInteraction = interactive && as !== "div";
  const handleKeyDown: KeyboardEventHandler<HTMLElement> = (event) => {
    onKeyDown?.(event);
    if (event.defaultPrevented || event.currentTarget !== event.target || !canUseRowInteraction || !onClick) return;
  ...
  return (
    <Comp
      ...
      title={title}
      onClick={canUseRowInteraction ? onClick : undefined}
      ...
      className={cn(
        "flow-row",
        canUseRowInteraction && "cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1",
