"use client";
import { forwardRef, useState, useRef, useEffect, type KeyboardEvent as ReactKeyboardEvent, type ReactNode } from "react";
...
function DrawerContent({
  ...
  const routePrefix = usePathname()?.startsWith("/demo") ? "/demo" : "/app";
  ...
  const postComment = async () => {
    ...
    if (!serverMode || !workspaceId || postingComment) return;
    setCommentError(null);
  ...
  }, [serverMode, item.id, initialFocusSection, reduceMotion]);
  ...
  const onDrawerKeyDown = (event: ReactKeyboardEvent<HTMLElement>) => {
    if (event.key !== "Tab" || presentation !== "board") return;
  ...
  const createSubtask = async () => {
    ...
    if (!title) {
      setSubtaskError("Add a subtask title before saving.");
      return;
    }
    if (!workspaceId || !serverMode || !onItemsReplaced) return;
    setSubtaskError(null);
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        aria-modal={presentation === "board" ? true : undefined}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        <dl className="mt-4 grid grid-cols-[72px_1fr] gap-y-2 text-[12px]">
          ...
          <dd>
            <input
              ...
              className="h-7 w-32 rounded border bg-card px-2 text-base outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1 sm:text-[12px]"
            />
          </dd>
          ...
          <dd>
            <select
              ...
              className="h-7 rounded border bg-card px-2 text-base outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1 sm:text-[12px]"
            >
        ...
        <div className="mt-5 border-t pt-4">
          ...
          {subtaskError && <p id="new-subtask-error" className="mb-2 text-[11px] text-red-600">{subtaskError}</p>}
          <ul className="space-y-1 text-[13px]">
            ...
            <li className="flex items-center gap-2 px-1 py-1">
              ...
              <input
                ...
                aria-describedby={subtaskError ? "new-subtask-error" : undefined}
                ...
                className="h-7 min-w-0 flex-1 rounded bg-transparent text-base outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1 sm:text-[13px]"
              />
              <button
                type="button"
        ...
        <CompactSection title="Steps" count={checklist.length > 0 ? `${checklist.filter((c) => c.done).length}/${checklist.length}` : undefined} defaultOpen={checklist.length > 0}>
          <ul className="space-y-2 text-[13px]">
            ...
            <li className="flex items-center gap-2.5 px-1 py-0.5">
              ...
              <input
                readOnly={readOnly}
                aria-label={checklist.length === 0 ? "Add checklist item" : "Add another checklist item"}
                ...
                className="h-7 flex-1 rounded bg-transparent text-base outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1 sm:text-[13px]"
              />
            </li>
          </ul>
        </CompactSection>
            ...
            <ul className="space-y-1">
              {linkedNotes.map((n) => (
                <li key={n.id} className="group flex items-center gap-2 rounded px-2 py-1.5 text-[13px] hover:bg-[var(--color-hover)]">
                  <FileText className="h-3.5 w-3.5 text-muted-foreground" />
                  <Link href={`${routePrefix}/notes?id=${encodeURIComponent(n.id)}`} className="min-w-0 flex-1 truncate text-foreground hover:underline">
                    {n.title}
                  </Link>
          ...
          <CompactSection title="Comments" count={comments.length} defaultOpen={initialFocusSection === "comments"} ref={commentsSectionRef}>
            ...
            <div className="mb-4">
              <textarea
                ...
                rows={3}
                className="w-full resize-y rounded border bg-card px-2 py-1.5 text-base outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1 sm:text-[12px]"
              />
              ...
              {commentError && <p id="task-comment-error" className="mt-1 text-[11px] text-red-600">{commentError}</p>}
  ...
  return (
    <label className="flex items-center gap-1.5">
      ...
      <select
        ...
        className="rounded bg-transparent text-base outline-none hover:underline focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1 sm:text-[12px]"
      >
