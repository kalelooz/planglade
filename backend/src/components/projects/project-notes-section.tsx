  ...
  return (
    <section className="grid min-h-[520px] min-w-0 grid-cols-1 gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <div className="min-w-0 rounded-md border bg-card">
        ...
        {error && <div id="project-note-error" role="alert" className="mx-4 mt-3 rounded border border-destructive/30 bg-destructive/10 px-3 py-2 text-[12px] text-destructive">{error}</div>}
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => {
                ...
                if (error === "Add a title before saving.") setError(null)
              }}
              placeholder="e.g. Meeting notes"
              aria-invalid={error === "Add a title before saving."}
              aria-describedby={error === "Add a title before saving." ? "project-note-error" : undefined}
              className="h-9 w-full rounded-md border bg-background px-2.5 text-base outline-none focus:border-ring sm:text-[13px]"
            />
          </label>
          <label className="block">
            ...
            <textarea
              ...
              rows={18}
              className="min-h-[360px] w-full resize-y rounded-md border bg-background px-3 py-2 text-base leading-6 outline-none focus:border-ring sm:text-[13px]"
            />
          </label>
        </div>
      </div>
