...
test("shared task rows only add row keyboard affordances for native row buttons", async () => {
  const source = await readProjectFile("src/components/lovable/flow-ui.tsx")

  assert.match(source, /const canUseRowInteraction = interactive && as !== "div"/)
  assert.match(source, /canUseRowInteraction && "cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-1"/)
  assert.match(source, /onClick={canUseRowInteraction \? onClick : undefined}/)
