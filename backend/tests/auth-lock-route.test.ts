...
function denyWorkspaceMembership() {
  ;(db.workspaceMember as typeof db.workspaceMember).findUnique = ((async (args: unknown) => {
    ...
    assert.ok(where?.workspaceId_userId?.userId)
    assert.notEqual(where?.workspaceId_userId?.userId, "user-a")
    return null
...
test("AUTH-LOCK-1: canonical dev user cannot read another workspace's work items, projects, or notes", async () => {
...
test("AUTH-LOCK-1: canonical dev user cannot create a task in another workspace", async () => {
  await runWithMocks(async () => {
    denyWorkspaceMembership()
    blockTransaction()
...
test("AUTH-LOCK-1: canonical dev user cannot update another workspace's task", async () => {
  await runWithMocks(async () => {
    denyWorkspaceMembership()
    blockTransaction()
