...
test("GET /auth/session creates dev scaffold session", async () => {
  await runWithMocks(async () => {
    ...
    ;(db.workspaceMember as typeof db.workspaceMember).findMany = ((async (args: { where?: { userId?: string; workspaceId?: string } }) => {
    ...
    const denied = await getAuthSession(new Request("http://localhost/api/auth/session?workspaceId=ws-other"))
