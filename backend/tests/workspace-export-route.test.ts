...
test("GET /workspace/export preserves the actor's saved views", async () => {
  await runWithMocks(async () => {
    ...
    ;(db.savedView as typeof db.savedView).findMany = ((async (args: unknown) => {
      assert.deepEqual((args as { where: unknown }).where, { workspaceId: "workspace-1", createdById: "actor-1" })
      return [{
        id: "view-1", workspaceId: "workspace-1", projectId: "project-1", createdById: "actor-1",
        name: "Launch risks", layout: "overview", groupBy: "project", orderBy: "priority",
        filters: { quick: ["blocked"] }, display: { version: 1, pinned: true }, isDefault: true,
    ...
    assert.equal(response.status, 200)
    assert.equal(payload.counts?.savedViews, 1)
    assert.equal(payload.savedViews?.length, 1)
    assert.deepEqual(payload.data?.savedViews?.[0], {
      id: "view-1", project: "project-1", name: "Launch risks", layout: "overview", groupBy: "project",
      orderBy: "priority", filters: { quick: ["blocked"] }, display: { version: 1, pinned: true }, isDefault: true,
    })
  })
})
...
test("POST /workspace/import-local restores saved presentation configuration", async () => {
  await runWithMocks(async () => {
    mockWorkspaceAccess("ADMIN")
    let createdViewData: unknown
    mockTransaction({
      workspaceMember: { findMany: async () => [{ userId: "actor-1" }] },
      project: { upsert: async () => ({ id: "project-real-1" }) },
      workItem: { findFirst: async () => null, create: async () => ({}), deleteMany: async () => ({}) },
      note: { findFirst: async () => null, create: async () => ({}), deleteMany: async () => ({}) },
      projectDoc: { findFirst: async () => null, create: async () => ({}), deleteMany: async () => ({}) },
      savedView: {
        findFirst: async () => null,
        updateMany: async () => ({}),
        create: async (args: unknown) => { createdViewData = (args as { data: unknown }).data; return { id: "view-real-1" } },
        deleteMany: async () => ({}),
      },
    })
    ...
    assert.equal(response.status, 201)
    assert.equal(payload.imported?.savedViews, 1)
    assert.deepEqual(createdViewData, {
      workspaceId: "workspace-1", projectId: "project-real-1", createdById: "actor-1", name: "Launch risks",
      layout: "overview", groupBy: "project", orderBy: "priority", filters: { quick: ["blocked"] },
      display: { version: 1, pinned: true }, isDefault: true,
    })
  })
})
