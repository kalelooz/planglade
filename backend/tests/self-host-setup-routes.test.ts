...
test("the three setup routes complete once and never replay recovery codes", async () => {
  ...
  const [{ GET }, discoveryRequest, claimRoute, completeRoute, completeRequest, setupService, setupSecurity] = await Promise.all([
    import("../src/app/api/auth/setup/route"),
    import("../src/lib/self-host-setup/discovery-request"),
    import("../src/app/api/auth/setup/claim/route"),
    import("../src/app/api/auth/setup/complete/route"),
    import("../src/lib/self-host-setup/complete-request"),
  ...
  try {
    ...
    try {
      unavailable = await discoveryRequest.getSetupDiscovery(
    ...
    const attackerResponse = await completeRequest.completeSetupRequest(
    ...
    const temporary = await completeRequest.completeSetupRequest(
    ...
    const preflightFailure = await completeRequest.completeSetupRequest(
      new Request("http://localhost:3000/api/auth/setup/complete", { method: "POST", headers: completionHeaders, body }),
      ((input, suppliedClaimant) => setupService.completeSetup(
        input,
        suppliedClaimant,
