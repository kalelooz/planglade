...
test("login evaluates provider availability at runtime instead of baking build-time secrets", async () => {
  const routeSource = await readProjectFile("src/app/login/page.tsx")

  assert.match(routeSource, /export const dynamic = "force-dynamic"/)
  assert.match(routeSource, /process\.env\.GOOGLE_CLIENT_ID/)
  assert.match(routeSource, /process\.env\.GOOGLE_CLIENT_SECRET/)
})
