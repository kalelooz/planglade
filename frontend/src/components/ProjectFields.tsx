      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            id={id}
            type="button"
            className="mt-1 inline-flex h-11 w-full items-center gap-2 lg:h-9 rounded-md border border-input bg-transparent px-3 text-left text-[13px] outline-none transition-colors hover:bg-accent/50 focus-visible:ring-2 focus-visible:ring-ring"
          >
        ...
        <PopoverContent className="w-auto p-0" align="start">
          ...
          {value && (
            <div className="border-t border-border p-2">
              <button type="button" onClick={() => { onChange(''); setOpen(false) }} className="h-11 w-full rounded-md text-[12px] lg:h-8 text-muted-foreground hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring">
                Clear date
              </button>
            </div>
          )}
  ...
  return (
    <div>
      ...
      <div className="mt-1 flex gap-2">
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <button id={id} type="button" className="inline-flex h-11 min-w-0 flex-1 items-center lg:h-9 gap-2 rounded-md border border-input bg-transparent px-3 text-left text-[13px] outline-none transition-colors hover:bg-accent/50 focus-visible:ring-2 focus-visible:ring-ring">
          ...
          <PopoverContent className="w-[18rem] max-w-[calc(100vw-2rem)] p-3 lg:w-56" align="start" collisionPadding={16}>
            <p className="mb-2 text-[12.5px] font-medium text-muted-foreground">Choose a color</p>
            <div className="grid grid-cols-5 gap-2 lg:grid-cols-5" role="group" aria-label="Project colors">
        ...
        <label className="relative grid h-11 w-11 shrink-0 lg:h-9 lg:w-10 cursor-pointer place-items-center rounded-md border border-input text-muted-foreground transition-colors hover:bg-accent hover:text-foreground has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring" title="Open advanced color picker">
...
export function ProjectIconField({ id, value, color, onChange }: { id: string; value: ProjectIconName; color: string; onChange: (value: ProjectIconName) => void }) {
  ...
  return (
    <div>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button id={id} type="button" className="mt-1 inline-flex h-11 w-full items-center gap-2 lg:h-9 rounded-md border border-input bg-transparent px-3 text-left text-[13px] outline-none transition-colors hover:bg-accent/50 focus-visible:ring-2 focus-visible:ring-ring">
            <SelectedIcon className="h-4 w-4 shrink-0" style={{ color }} aria-hidden />
            <span className="truncate">{selected.label}</span>
            <ChevronDown className="ml-auto h-3.5 w-3.5 text-muted-foreground" aria-hidden />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[22.5rem] max-w-[calc(100vw-2rem)] p-3 lg:w-[17rem]" align="start" collisionPadding={16}>
          <p className="mb-2 text-[12.5px] font-medium text-muted-foreground">Choose an icon</p>
          <div className="grid grid-cols-5 gap-1 sm:grid-cols-7" role="group" aria-label="Project icons">
