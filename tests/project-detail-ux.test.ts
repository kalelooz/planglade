...
test("project overview and tasks rows use flexible title space before metadata", async () => {
  ...
  assert.match(workItemRowSource, /grid-cols-\[auto_minmax\(0,1fr\)\]/)
  assert.match(workItemRowSource, /sm:grid-cols-\[auto_minmax\(22rem,1fr\)_96px_minmax\(7rem,9rem\)_112px_32px\]/)
  const titleIndex = workItemRowSource.indexOf("{item.title}")
  const priorityIndex = workItemRowSource.indexOf("<PriorityIndicator")
  const assigneeIndex = workItemRowSource.indexOf("<Avatar")
  const dueIndex = workItemRowSource.indexOf("{dueLabel}")
  const actionsIndex = workItemRowSource.indexOf("<DropdownMenu>")
  assert.ok(titleIndex < priorityIndex && priorityIndex < assigneeIndex && assigneeIndex < dueIndex && dueIndex < actionsIndex)
  assert.match(workItemRowSource, /Parent: \{parentTask\.title\}/)
  assert.match(workItemRowSource, /subtasks/)
  assert.match(workItemRowSource, /completed \? "text-muted-foreground line-through" : "text-foreground"/)
