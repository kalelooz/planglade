...
test("Home task rows use compact title-first metadata order", async () => {
  ...
  assert.match(rowSource, /grid-cols-\[18px_minmax\(0,1fr\)_auto_20px\]/)

  const rowButtonIndex = rowSource.indexOf("onClick={onOpen}")
  const titleIndex = rowSource.indexOf("data-home-row-title")
  const dueIndex = rowSource.indexOf("data-home-row-metadata")
  const priorityIndex = rowSource.indexOf("<Flag aria-label")

  assert.ok(rowButtonIndex > 0, "Home task preview row button missing")
  assert.ok(titleIndex > 0, "title must be available on the preview row")
  assert.ok(dueIndex > titleIndex, "due date must follow title")
  assert.ok(priorityIndex > dueIndex, "priority flag must follow due date")
